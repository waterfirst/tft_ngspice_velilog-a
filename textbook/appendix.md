...
| 파형에서 잰 duty·평균이 이상하게 큼 | ngspice 시간점은 에지 근처에 몰림 → 점 개수 ≠ 시간 | `np.trapezoid`로 시간 가중 | 9장 |
| 가속 타이밍에서는 잔상이 작게 보임 | 프레임 길이 ≈ 트랩 시정수라 한 프레임 안에서 평균됨 | 결론은 실타이밍으로 확인 | 9장 |
...
| **히스테리시스 (여기서)** | 과거 바이어스 이력이 현재 문턱전압에 남는 가역 효과. 9장 모델은 트랩 상태 1개(`KH`, `TAUH`) |
| **트랩 시정수 τ** | 트랩 상태가 새 바이어스를 63% 따라가는 시간 (`TAUH`) |
| **OBS (on-bias stress)** | 기록 직전 구동 TFT에 강한 온바이어스를 잠깐 걸어 이전 계조 기억을 지우는 구동. 예시 8T1C의 T8 |
| **E1** | 계조 전환 후 첫 프레임 전류가 직전 계조에 따라 달라지는 정도 `|I₁(저계조 출발) − I₁(고계조 출발)| / I_ss` |
| **N_settle** | 전류가 정상상태 ±1% 안에 처음 들어오는 프레임 번호 |
