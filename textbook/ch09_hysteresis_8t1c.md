...
| 타이밍 | 60 Hz 프레임, 초기화·기록 펄스 각 200 µs | 60 Hz 프레임, 초기화·기록 펄스를 **각 100 µs로 단축** + 비교용 **가속 1 ms 프레임** |
| 지표 | 2프레임째 발광 구간 평균 전류 | 계조 전환 후 **프레임 1…8 각각의** 평균 전류 → E1, N_settle, R_ss |
...
## 결과 5 — T7·T8 스트레스는 가정과 달랐다: duty는 맞고 \|Vgs\|가 틀렸다

7장 타이밍(초기화 펄스 200 µs)의 60 Hz 파형에서 시간 가중으로 측정 (`code/05_full_8t1c/results/stress_probe.csv`, 11장)

| | 가정 (7장) | 파형에서 측정 | 스트레스 지수 duty·(\|Vgs\|/5)² |
|---|---|---|---|
| duty | 0.012 | **0.0126** (= 200 µs + 에지 절반씩 / 16.667 ms) | |
| T7 \|Vgs\| | 10 V | **3.42 V** | 0.048 → **0.0059** (약 1/8) |
| T8 \|Vgs\| | 10 V | **11.85 V** | 0.048 → **0.071** (약 1.5배) |

duty 가정은 맞았고 \|Vgs\| 가정이 틀렸다. T7은 게이트가 −7 V일 때 소스(애노드)가 이미 VINIT 근처(−3.5 V)로 끌려가 있어 \|Vgs\|가 작고, T8은 소스가 VOBS 5 V에 고정돼 있어 \|Vgs\|가 12 V에 가깝다. (이 장의 스윕은 펄스를 100 µs로 줄였으므로 duty가 0.0061로 절반이다.) **스트레스는 넷리스트를 한 번 돌려 파형에서 읽어야** 한다 — 5장 "비유가 깨지는 곳 3"을 실제로 해본 결과다.

> **에이전트가 만든 함정 (8장 연결)**
> 이 장의 첫 코드는 AI 코딩 에이전트가 썼다. duty를 `(S1<0인 점 개수) / (전체 점 개수)`로 셌고 **0.082**가 나왔다. ngspice는 스위칭 에지 근처에 시간점을 촘촘히 찍으므로 점 개수는 시간에 비례하지 않는다. 시간 가중(`np.trapezoid`)으로 바꾸자 0.0061(이 장의 100 µs 펄스 기준) — **13.5배 과대**였다. 같은 코드에서 측정한 T7·T8 파라미터를 만들어 놓고 회로 카드에는 **옛 값을 넣는** 버그도 있었다. 둘 다 에러 없이 그럴듯한 숫자를 냈다. "측정값 = 100 µs / 16.667 ms"처럼 **손으로 검산 가능한 값과 비교**하는 습관이 잡아냈다.
